...
void ImageSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (label_type == ImageDataParameter_LabelType_PIXEL) {
      ...
      cv_img.convertTo(cv_img, CV_32SC1);
	...
	DLOG(INFO) << "Fail to load seg: " << root_folder + lines_[lines_id_].second;
    ...
    else if (label_type == ImageDataParameter_LabelType_BOX) {
      // bounding boxes as annotation
      cv::Mat cv_img = ReadImageToCVMat(root_folder + lines_[lines_id_].second,
					    new_height, new_width, true);
	  //printf("number of channels %d\n", cv_img.channels());
	  //printf("data type %d\n",cv_img.type());
	  
	  //cv_img.convertTo(cv_img, CV_32SC3);
	  cv::Mat int32box = cv::Mat::zeros(cv_img.rows, cv_img.cols, CV_32SC1);
	  
	  for(int x=0;x<cv_img.cols;x++){
	    for(int y=0;y<cv_img.rows;y++){
	      uchar b = cv_img.data[cv_img.channels()*(cv_img.cols*y + x) + 0];    
          uchar g = cv_img.data[cv_img.channels()*(cv_img.cols*y + x) + 1];
          uchar r = cv_img.data[cv_img.channels()*(cv_img.cols*y + x) + 2];
          int32box.at<int>(y,x) = (int)(r) + (int)(g) * 256 + (int)(b) * 256 * 256;
        }
      }
      ...
      //cv_img_seg.push_back(ReadImageToCVMat(root_folder + lines_[lines_id_].second,
		//			    new_height, new_width, false));
      //printf("newheight and newwidth %d %d\n", new_height, new_width);
      //printf("rows and columns %d %d\n", int32box.rows, int32box.cols);
      cv_img_seg.push_back(int32box);
      if (!cv_img_seg[1].data) {
	DLOG(INFO) << "Fail to load seg: " << root_folder + lines_[lines_id_].second;
      }
    }
