...
class SoftmaxLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        data = self.forward_theano(bottom[0].data[...])
        # print "D2:", np.linalg.norm(np.ndarray.flatten(data))
        top[0].data[...] = data

    def backward(self, top, prop_down, bottom):
        grad = self.backward_theano(bottom[0].data[...], top[0].diff[...])
        # print "G2:", np.linalg.norm(np.ndarray.flatten(grad))
        bottom[0].diff[...] = grad
...
class CRFLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        im = bottom[1].data[...]
        if (bottom[0].data.shape[2] != im.shape[2]):
