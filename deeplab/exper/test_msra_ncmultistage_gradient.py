...
solver = caffe.SGDSolver('msra10k/config/deeplab_vgg16/solverncmultistage_train.prototxt')
solver.net.copy_from('msra10k/model/deeplab_vgg16/train_iter_2000.caffemodel')
training_net = solver.net

solver.step(1)

print 'accuracy', training_net.blobs['accuracy'].data
print 'accuracyNC', training_net.blobs['accuracyNC'].data
print training_net.blobs['fc8_interp'].data.shape
...
fc8_interp_data=training_net.blobs['fc8_interp'].data
fc8_interp_data=fc8_interp_data[imgid,...,...,...]
fc8_interp_diff=training_net.blobs['fc8_interp'].diff
fc8_interp_diff=fc8_interp_diff[imgid,...,...,...]
...
for i in range(2):
    plt.subplot(1,2,i)
    plt.imshow(fc8_interp_diff[i,...,...]);
    plt.title('Input diff channel '+`i`);
    plt.colorbar()
    #plt.savefig('pythonoutput/' + str(i) + 'prob' + '.png')
    plt.show(block=False)
    #plt.close(fig)

multistagencpred=training_net.blobs['multistagencpred'].data
multistagencpred=multistagencpred[imgid,...,...,...]
multistagencpred_diff=training_net.blobs['multistagencpred'].diff
multistagencpred_diff=multistagencpred_diff[imgid,...,...,...]

fig=plt.figure()
for i in range(2):
    plt.subplot(1,2,i)
    plt.imshow(multistagencpred[i,...,...]);
    plt.title('Output data channel '+`i`);
    plt.colorbar()
    #plt.savefig('pythonoutput/' + str(i) + 'prob' + '.png')
    plt.show(block=False)
    #plt.close(fig)
...
for i in range(2):
    plt.subplot(1,2,i)
    plt.imshow(multistagencpred_diff[i,...,...]);
    plt.title('Output diff channel '+`i`);
    plt.colorbar()
    #plt.savefig('pythonoutput/' + str(i) + 'prob' + '.png')
    plt.show(block=False)
    #plt.close(fig)
...
for i in range(2):
    plt.subplot(1,2,i)
    plt.imshow(multistagencpred_diff[i,...,...]-fc8_interp_diff[i,...,...]);
    plt.title('Output diff - input diff channel '+`i`);
    plt.colorbar()
    #plt.savefig('pythonoutput/' + str(i) + 'prob' + '.png')
    plt.show(block=False)
    #plt.close(fig)
