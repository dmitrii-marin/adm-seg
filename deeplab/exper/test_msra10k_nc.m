...
% initialization
load(['msra10k/features/deeplab_largeFOV/' whichset '/fc8/' imgname '_blob_0.mat']); % data is of WxHxC
...
imwrite(encodesegmentation( img, a ),['clusteringnetwork/' imgname '_init.png']);
...
imwrite(encodesegmentation( img, a ),['clusteringnetwork/' imgname '_kkm.png']);

% NC network
load(['msra10k/featuresnc/deeplab_vgg16/' whichset '/fc8/' imgname '_blob_0.mat']); % data is of WxHxC
probmap = data(1:(min(W,size(data,1))), 1:(min(H,size(data,2))),:);
probmap = canonicalForm( probmap );
[~,a] = max(probmap,[],3);
%figure,imagesc(a);
...
imwrite(encodesegmentation( img, a ),['clusteringnetwork/' imgname '_network.png']);
